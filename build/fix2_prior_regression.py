...
def marks_and_cooldown():
    ...
    primary = NS(DispelIfActive=lambda: None)
    secondary = NS(DispelIfActive=lambda: None)
...
def stale_host_identity():
    ...
    host = NS(ImportState=imported, PrepareSwap=lambda: rejected.append('stop'),
              DispelIfActive=lambda: rejected.append('dispel'))
...
if __name__ == '__main__':
    for name, test in [
        ...
        check(name, test)
    (ROOT / 'build/fix2-prior-regression.json').write_text(json.dumps({
