...
def install():
    ...
    runpy.run_path(str(WORK/'build/fix18e_isolated.py'))['install'](str(WORK))
    allowed = (WORK/'build', WORK/'package/Elements Spellblade')
    def audit(event, args):
        if event.startswith('socket.'):
            raise PermissionError('Round18e: network forbidden')
        ...
        if writing:
            ...
            if not any(path.is_relative_to(root) for root in allowed):
                raise PermissionError('Round18e: write outside granted output trees: '+str(path))
...
def main():
    ...
    digest = runpy.run_path(str(WORK/'build/fix18e_isolated.py'))['digest_tree']
