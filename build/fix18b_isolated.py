...
The release recipe and every existing check still execute. No release file is
opened for writing. Compiler subprocess arguments are redirected too. PEX build
...
def isolated(b):
    work=b.WORK.resolve()
    snap=work/'.codex/pre-fix18b-snapshot'
    ...
    assert digest_tree(release)==baseline, 'Release changed since snapshot'
    ...
    assert digest_tree(release)==baseline, 'Live release package was modified'
    ...
    report=dict(build_exit_code=0,release_files=len(baseline),original_release_byte_identical=True,
    ...
    lines=[f'PRESERVATION ok: {len(baseline)} release files untouched; 36 rebuilt files byte-identical + {len(reuse)} decoded PEX equivalent; original PEX retained for identical release assembly',
           'SCOPE ok: full-build writes isolated under .codex/pre-fix18b-snapshot; only opt-in probe artifacts published']
