...
def run():
    ...
    assert len(proof)==60 and sum(p['G'] for p in proof)==55
    ...
    # Actual shared producer outputs scale linearly with owning tree G exactly once.
    producers=[('ESSBElem2','FissureArmor',3,120),('ESSBElem2','RockArmorPerLayer',3,20),('ESSBElem2','QuakeStamina',3,5.8)]
    for script,fn,tree,base in producers:
        for level in [1,10,100]:
            ctl=Ctl(settings);ctl.level=level;reg=make(ROOT/'src',ctl)
            assert math.isclose(getattr(reg[script],fn)(ctl),base*(1+.05*level)),(script,fn,level)
    ...
    for count in range(1,7):
        ...
        assert math.isclose(heal,min(25*count,100)*ctl.GetDamageMult(7)*ctl.GLevel(6))
        assert len(hits)==count
        judge.append({'targets':count,'base_total':min(25*count,100),'scaled_heal':heal})
    ...
    print(f'FIX7 ok: existing non-quest IDs unchanged; baseline={len(baseline)}; 5 appended float GLOBs; category execution + G once + Judge cap + fixed layout + encodings verified')
    return report
