...
| 02 | 命中流血目標吸血（固定 B 基準） | B_max 10 × 吸血比 × 血位命中倍率；基礎滿血 0.65，低血 3；吸血主線滿點滿血 2.6、低血 3.9 | B_max ×5 × 原吸血比 × 血位命中倍率 | `native/include/HitMath.h:AddBloodLeech`（行 422） |
...
| 05 | 低點數沉默及首領折半 | 1+0.2×rank，四捨五入；rank1–2 實際 1 秒，首領最少1秒 | 1+0.2×rank，原四捨五入及首領折半（保留） | `native/include/HitMath.h:SilenceSeconds`（行 597） |
...
| 11 | 命中削耐的一半回耐 | 0.25 × rank，滿點 3.75／擊 | 1.5 × rank | `native/include/HitMath.h:AddFlatHitNodes`（行 450） |
...
| 16 | 命中回耐 | 5／擊 | 25／擊 | `native/include/HitMath.h:AddFlatHitNodes`（行 459） |
...
| 20 | 每擊固定削耐 | 0.5 × rank；滿點 7.5 | 3 × rank | `native/include/HitMath.h:AddFlatHitNodes`（行 450） |
...
| 02 | 5 | `const float heal = procMagnitude * LeechRatio(p, nodes) * t.multRecovery;`；`native/include/HitMath.h:AddBloodLeech`（行 422） | Cast::kHeal / kBloodGuard；G 在附傷強度（RollProc）裡乘過一次 | 1 |
...
| 05 | 豁免 | `const float wanted = std::min(1.0f + 0.2f * static_cast<float>(nodes.Rank(node::kNoFormSilence)), static_cast<float>(kMaxSilenceSeconds)) +`；`native/include/HitMath.h:SilenceSeconds`（行 597） | Cast::kSilence（固定時長法術）；G 豁免 | 0 |
...
| 11 | 3 | `const float cut = 3.0f * static_cast<float>(nodes.Rank(node::kEarthStaminaCut)) * TreeG(t, TreeOf(kEarth));`；`native/include/HitMath.h:AddFlatHitNodes`（行 450） | Cast::kDrainStamina；回耐 ×0.5 另一步 | 1 |
...
| 16 | 4 | `plan.Add({ Cast::kRestoreStamina, 25.0f * TreeG(t, TreeOf(kWind)) * t.multRecovery });`；`native/include/HitMath.h:AddFlatHitNodes`（行 459） | Cast::kRestoreStamina | 1 |
...
| 20 | 3 | `const float cut = 3.0f * static_cast<float>(nodes.Rank(node::kEarthStaminaCut)) * TreeG(t, TreeOf(kEarth));`；`native/include/HitMath.h:AddFlatHitNodes`（行 450） | Cast::kDrainStamina；Cast::kRestoreStamina 取同一 cut ×0.5 | 1 |
