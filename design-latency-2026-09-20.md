...
| **N6**（最低）（2026-09-25 使用者決定：要做） | 每秒計時（維持費、火源代價、水回魔、環境）、熱鍵輸入 sink | 全部（Papyrus 每秒一次已夠便宜） | 只有在 N1～N5 後仍量到 Papyrus 每秒工作造成延遲時才做 | — | 若做：背景計時執行緒每 tick 以 SKSE AddTask 送一個主執行緒工作、熱鍵走輸入事件 sink，都不需要 hook（見 build/native-verification-3.md 第 11、12 項），所以最後且可不做（2026-09-25 使用者決定：要做） | — |
