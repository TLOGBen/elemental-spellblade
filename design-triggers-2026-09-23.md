...
| 斷咒：打斷施法中的敵人，每 5 秒一次 | 命中當下 | `NoForm.OnManaBreak`（IsCasting＋`Ctl.OnAnimationEvent` 記錄） | 〔已定 2026-09-23〕DLL命中：命中時讀目標施法狀態＋中斷（同雷滿格法術麻痺的技術，待查證：讀取目標施法狀態、中斷施法） | 目標 5 秒冷卻效果 | N4 |
...
| 反咒：帶破魔印的敵人施法 → 受施法消耗 100% 真傷 | 其他（目標施法） | `ESSBCounter`／`Ctl.OnAnimationEvent`→`NoForm.OnCounterSpell` | 〔已定 2026-09-23〕DLL命中：改走 DLL 的施法事件（待查證：施法事件 spell-cast event，見 3.2） | 破魔印（目標效果） | N4 |
...
| 暴擊：雷是唯一會暴擊的元素，暴擊率 = 基礎 5% + 每格電荷 +2%，套在附傷與放電；一般 ×1.5、重擊放電 ×2.5〔已定 2026-09-23〕 | 命中當下 | 無（現況不存在） | DLL命中 | 電荷效果強度 | N2 |
...
| 放電（終焉）：電荷×30% B_max、削魔 50%、跳 2 人 40%、吃重擊倍率；滿格時的重擊放電必定暴擊 ×2.5〔已定 2026-09-23，取代原「滿格 ×2」，見 2.1 暴擊判定〕 | 狀態結束（切／融／期） | `React.EndShock`→`Elem.DischargeAll`／`Discharge`（讀 LastHitWasPower） | DLL（被切命中；過期事件；融斷掃描）；暴擊判定併入 N2 | 電荷效果強度 | N3／N5（暴擊判定 N2） |
...
| # | 機制 | 選項 | 玩家感覺 | 成本 | 建議 |
|---|---|---|---|---|---|
| 1 | **血的吸血**（1.1：命中流血目標，占附傷） | (a) 命中當下，DLL 算附傷時一起回血（N2）；(b) 血痕過期或血潮結算時回；(c) 血痕每跳回一點 | (a) 砍一刀回一口，跟血位曲線同步，最直覺；(b) 延遲、跟打擊脫節；(c) 停手也在回，比較像「吸血鬼光環」 | (a) 零額外面；(b) 要過期事件；(c) 要 DLL 每秒點，或每層 DoT 帶一顆「治療施法者」效果（引擎做不到） | **已定 2026-09-23**：基礎吸血走 (a)（N2，DLL）；血潮保留結算時的治療；(c)「血痕每跳回血」列為候選，取決於放血需要的 DLL 每秒點做不做得出來 |
...
| 6 | **雷神**（同調三段電荷滿層自動放電＋回魔） | (a) 命中當下：電荷一到滿就放（對被命中的目標）；(b) 每秒輪詢，對最近的帶雷印記目標（現況 `Elem.OnTick`） | (a) 第 6／10／13 下同幀爆，節奏清楚；(b) 滿了之後最多等 1 秒、目標可能不是你在打的人 | (a) 零；(b) 需要每秒點或 Papyrus 每秒呼叫 DLL | **已定 2026-09-23**：(a)，也把「放電對象」定為被命中目標；雷同時是全模組唯一會暴擊的元素（見 2.5、alter 決定 25），暴擊判定與放電同一刀處理 |
...
| 11 | **瘴氣／瘟疫／毒霧的「+劑」**（即「元素狀態每跳觸發的事件」這一類） | (a) 引擎披風每秒對鄰居套一顆小法術，DLL 接「效果套用事件」把它併進鄰居的單一中毒；(b) DLL 每秒點；(c) Papyrus 每秒呼叫 DLL 原生函式 | 三者手感相同（每秒一次） | (a) 新的事件 sink（不在任何切片）；(b) 每秒點；(c) 回到每秒掃描 | 先查證 (a)；不行走 (c)。注意 latency 4.11 說「引擎自己傳」，但單一成長效果的「+劑」引擎自己做不到。**元素狀態每跳觸發事件：使用者暫緩〔2026-09-23〕**——先不做，不寫進設計，留在這裡當候選 |
...
15. **反咒**（目標施法時）：沒有任何 DLL 切片處理「敵人施法」事件，只能留 Papyrus（現況靠動畫事件）。〔已定 2026-09-23：見 2.1，反咒與斷咒改走 DLL（N4）——反咒走施法事件、斷咒走命中時讀目標施法狀態＋中斷；三項 API 都待查證，見 latency 4.11〕
